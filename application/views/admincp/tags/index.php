...
<div class="content">
<form action = '<?php echo base_url('admincp/tags/delete');  ?>' method = 'post'  name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
    ...
    <?php }else{
		 foreach ($info as $item) { 
			...
			$urledit = base_url('admincp/tags/edit/'.$item->Id);
			$urldel = base_url('admincp/tags/delete/'.$item->Id);
	?>
      <tr>
            ...
            <a href = '<?php echo $urledit?>'><?php echo $item->title_vn; ?></a>
			</div
			></td>
			...
			<?php 
            if($item->ticlock == "1"){
                echo "<div id = '".$item->Id."'><a href = 'javascript:ticlockactive(\"tags_model\",\"ticlock\",\"".$item->Id."\",\"0\");' title = 'Bỏ khóa'><img src = '".$imgremove."'></a></div>";
            }else{
                echo "<div id = '".$item->Id."'><a href = 'javascript:ticlockactive(\"tags_model\",\"ticlock\",\"".$item->Id."\",\"1\");' title = 'Khóa tin'><img src = '".$imgtick."'></a></div>"; 
            }
            ?>
            </td>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:confirmSave('Bạn có chắc muốn lưu lại','<?php echo  base_url('admincp/tags/save');?>')" class="button">Save</a>
</div>
</form>
</div>
