...
<a href='<?php echo base_url('admincp/catelog/add');  ?>' class="button" >
<img src = '<?php echo ADMIN_PATH_IMG; ?>icon-16-plus.png'> Thêm mới</a>
</div>
<form action = '<?php echo base_url('admincp/catelog/delete');  ?>' method = 'post'  name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
    ...
    <?php }else{
		function TreeCatnews($pid,$info,$text="--") {
		 foreach ($info as $item) { 
			...
			$urledit = base_url('admincp/catelog/edit/'.$item['Id']);
			$urldel = base_url('admincp/catelog/delete/'.$item['Id']); 
	?>
      <tr>
			<td align = 'center'><input type="checkbox" id = 'check_list' name="check_list[]" value="<?php echo $item['Id'];?>"><br></td>
			<td><?php echo $item['Id']; ?></td>
            <td align="left"><a href = '<?php echo $urledit?>' style='<?php echo $cls; ?>'><?php echo $text.$item['title_vn']; ?></a></td>
            <td align="left"><?php echo base_url($item['alias']); ?></td>
			...
			<?php 
            if($item['home'] == 1){
                echo "<div id = 'home".$item['Id']."'><a href = 'javascript: hideshow(\"catelog_model\",\"home\",\"".$item['Id']."\",\"0\",\"home".$item['Id']."\");' title = 'Bỏ khóa'><img src = '".ADMIN_PATH_IMG."icon-16-default.png'></a></div>";
            }else{
                echo "<div id = 'home".$item['Id']."'><a href = 'javascript: hideshow(\"catelog_model\",\"home\",\"".$item['Id']."\",\"1\",\"home".$item['Id']."\");' title = 'Khóa user'><img src = '".ADMIN_PATH_IMG."icon-16-nondefault.png'></a></div>"; 	 
            }
            ?>
            </td>
			<td align = 'center'>
			<?php 
            if($item['hot'] == 1){
                echo "<div id = 'hot".$item['Id']."'><a href = 'javascript: hideshow(\"catelog_model\",\"hot\",\"".$item['Id']."\",\"0\",\"hot".$item['Id']."\");' title = 'Bỏ khóa'><img src = '".ADMIN_PATH_IMG."icon-16-default.png'></a></div>";
            }else{
                echo "<div id = 'hot".$item['Id']."'><a href = 'javascript: hideshow(\"catelog_model\",\"hot\",\"".$item['Id']."\",\"1\",\"hot".$item['Id']."\");' title = 'Khóa user'><img src = '".ADMIN_PATH_IMG."icon-16-nondefault.png'></a></div>"; 	 
            }
            ?>
            </td>
			 <td align = 'center'>
			<?php 
            if($item['ticlock'] == "1"){
                echo "<div id = '".$item['Id']."'><a href = 'javascript:ticlockactive(\"catelog_model\",\"ticlock\",\"".$item['Id']."\",\"0\");' title = 'Bỏ khóa'><img src = '".$imgremove."'></a></div>";
            }else{
                echo "<div id = '".$item['Id']."'><a href = 'javascript:ticlockactive(\"catelog_model\",\"ticlock\",\"".$item['Id']."\",\"1\");' title = 'Khóa tin'><img src = '".$imgtick."'></a></div>"; 
            }
            ?>
            </td>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:confirmSave('Bạn có chắc muốn lưu lại','<?php echo  base_url('admincp/catelog/save');?>')" class="button">Save</a>
</div>
</form>
</div>
