...
<a href='<?php echo base_url('admincp/location/add');  ?>' class="button" >
<img src = '<?php echo ADMIN_PATH_IMG; ?>icon-16-plus.png'> Thêm mới</a>
</div>
<form action = '<?php echo base_url('admincp/location/delete');  ?>' method = 'post'  name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
    ...
    <?php }else{
		 foreach ($info as $item) { 
			...
			$urledit = base_url('admincp/location/edit/'.$item->id);
			$urldel = base_url('admincp/location/delete/'.$item->id); 
	?>
      <tr>
			...
			<?php 
            if($item->ticlock == "1"){
                echo "<div id = '".$item->id."'><a href = 'javascript:ticlockactive(\"location_model\",\"ticlock\",\"".$item->id."\",\"0\");' title = 'Bỏ khóa'><img src = '".$imgremove."'></a></div>";
            }else{
                echo "<div id = '".$item->id."'><a href = 'javascript:ticlockactive(\"location_model\",\"ticlock\",\"".$item->id."\",\"1\");' title = 'Khóa tin'><img src = '".$imgtick."'></a></div>"; 
            }
            ?>
            </td>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:confirmSave('Bạn có chắc muốn lưu lại','<?php echo  base_url('admincp/location/save');?>')" class="button">Save</a>
</div>
</form>
</div>
