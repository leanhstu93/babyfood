...
$images = null;
if(!empty($info)){ 
	$title_vn  = $info[0]['title_vn'];
	$route  = $info[0]['route'];
	$parenid  = $info[0]['parentid'];
	$sort  = $info[0]['sort'];
	$images  = $info[0]['images'];
	$ticlock  = $info[0]['ticlock'];
}
?>
...
<tr>
    ...
    <td>
        <select name = 'parentid'>
			...
			<option value="1" <?php if(!empty($parenid)){ if($parenid=='1') echo 'selected';} ?> >- - Hệ thống - -</option>
            <option value="2" <?php if(!empty($parenid)){ if($parenid=='2') echo 'selected';} ?> >- - Quản lý  nội dung - -</option>
            <option value="3" <?php if(!empty($parenid)){ if($parenid=='3') echo 'selected';} ?>>- - Quản lý cửa hàng - -</option>
            <option value="4" <?php if(!empty($parenid)){ if($parenid=='4') echo 'selected';} ?>>- - Thống kê - -</option>
        </select>
...
<tr>
    ...
    <td> <input type="text" name="title_vn" value="<?php echo set_value('title_vn',$title_vn) ?>"  style="width:400px"> 
    <?php echo form_error('title_vn'); ?>
    </td>
</tr>
<tr>
	<td class = 'title_td' >Route</td>
    <td> <input type="text" name="route" value="<?php if(!empty($route)) echo $route; ?>"  style="width:400px">
    	<?php echo form_error('images'); ?>
     </td>
</tr>
<tr>
	<td class = 'title_td' >Thứ tự</td>
    <td> <input type="text" name="sort" value="<?php if(!empty($sort)) echo $sort; ?>"  style="width:400px"> </td>
</tr>

<tr>
	<td class = 'title_td' >Hình ảnh</td>
    <td> <input type="text" name="images"  id="id_image"  value="<?php if(!empty($images)) echo $images; ?>"  style="width:400px"> </td>
</tr>
<tr>
...
<tr>
    ...
    <td> <input type="checkbox" value="1" name="ticlock"  <?php if(!empty($ticlock)){ if($ticlock==1) echo 'checked'; } ?> /> </td>
</tr>
<tr>
<td  ></td>
    <th align = 'left'>
