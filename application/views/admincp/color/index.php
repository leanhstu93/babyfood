...
<a href='<?php echo base_url('admincp/color/add');  ?>' class="button" >
<img src = '<?php echo ADMIN_PATH_IMG; ?>icon-16-plus.png'> Thêm mới</a>
<form action="<?php echo base_url('admincp/color/index'); ?>" method="get">
...
<form action = '<?php echo base_url('admincp/color/delete');  ?>' method = 'post'  name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
    ...
    <?php }else{
		 foreach ($info as $item) { 
			...
			$urledit = base_url('admincp/color/edit/'.$item['Id']);
			$urldel = base_url('admincp/color/delete/'.$item['Id']); 
	?>
      <tr>
			<td align = 'center'><input type="checkbox" id = 'check_list' name="check_list[]" value="<?php echo $item['Id'];?>"><br></td>
			<td><?php echo $item['Id']; ?></td>
            <td align="left"><a href = '<?php echo $urledit?>'><?php echo $item['title_vn']; ?></a></td>
            <td><span style="background-color:<?php echo $item['color']; ?>" class="coxdemo"></span></td>
            <td  width="100"><input type="text" size="1" value="<?php echo $item['sort']; ?>" name="sort[<?php echo  $item['Id']; ?>]"  style="text-align:center"/></td>
			 <td align = 'center'>
			<?php 
            if($item['ticlock'] == "1"){
                echo "<div id = '".$item['Id']."'><a href = 'javascript:ticlockactive(\"color_model\",\"ticlock\",\"".$item['Id']."\",\"0\");' title = 'Bỏ khóa'><img src = '".$imgremove."'></a></div>";
            }else{
                echo "<div id = '".$item['Id']."'><a href = 'javascript:ticlockactive(\"color_model\",\"ticlock\",\"".$item['Id']."\",\"1\");' title = 'Khóa tin'><img src = '".$imgtick."'></a></div>"; 
            }
            ?>
            </td>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:confirmSave('Bạn có chắc muốn lưu lại','<?php echo  base_url('admincp/color/save');?>')" class="button">Save</a>
</div>
</form>
</div>
