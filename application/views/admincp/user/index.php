<div class="main_area">
    ...
    <td> Quản lý nội dung / Thành viên  </td>
    </tr>
    </tbody>
    </table>
    </div>
...
<form method="get" action = "<?php echo base_url('admincp/user');?>" >
...
<form action = '<?php echo base_url('admincp/user/delete');  ?>' method = 'post'  name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
        ...
        <th width="150"><span onClick="javascript:sortOrder('date','<?php if($this->session->userdata('sortuser')=='date asc') echo 0; else echo 1; ?>');" style="cursor:pointer">Ngày đăng ký</span></th>
    ...
    <?php }else{
		 foreach ($info as $item) { 
			...
			$urledit = base_url('admincp/user/edit/'.$item->id);
			$urldel = base_url('admincp/user/delete/'.$item->id); 
			$urllogin = base_url('admincp/user/loginUser/'.$item->id); 
	?>
    <tr>
        <td align = 'center'><input type="checkbox" id = 'check_list' name="check_list[]" value="<?php echo $item->id;?>"><br></td>
        <td><?php echo $item->id; ?></td>
        <td align="left"><a href = '<?php echo $urledit;?>' title = 'Sửa'><?php echo $item->fullname; ?></a></td>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:confirmSave('Bạn có chắc muốn lưu lại','<?php echo  base_url('admincp/user/save');?>')" class="button">Save</a>
</div>
</form>
</div>
