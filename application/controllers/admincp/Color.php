...
class Color extends CI_Controller  {
	...
	{
		$temp['template']='admincp/color/index'; 
		$temp['idmenu']=3;
		...
		if($tukhoa!=""){
			$sql = "SELECT * FROM mn_color WHERE ( title_vn like  '%".$tukhoa."%') OR ( color like  '%".$tukhoa."%')     ORDER BY  sort ASC"; 
			$sql_toal = "SELECT COUNT(mn_color.Id) AS total FROM  mn_color  WHERE ( title_vn like  '%".$tukhoa."%') OR ( color like  '%".$tukhoa."%')    ORDER BY  sort ASC "; 
		}else{
			$sql = "SELECT * FROM mn_color     ORDER BY  sort ASC"; 
			$sql_toal = "SELECT COUNT(mn_color.Id) AS total FROM  mn_color    ORDER BY  sort ASC "; 

		}
		$link	=	base_url('admincp/color/index?tukhoa='.$tukhoa.'&idcat='.$idcat.'&p=');
	...
	public function add()
	{
		...
		if($this->input->post('save'))
		{
			if($this->form_validation->run() == TRUE  )
			{
				...
				$result = $this->color_model->add();
				$url = base_url('admincp/color');
				redirect($url);
			}
		}
		$temp['template']='admincp/color/add'; 
		$this->load->view("admincp/layout",$temp); 
	}
	public function edit($id)
	{
		$id = $this->uri->segment(4);
		$info = $this->color_model->get_where($id);
		...
		if($this->input->post('save'))
		{
			if($this->form_validation->run() == TRUE  )
			{
				$result = $this->color_model->update($id,NULL,true);
				redirect(base_url('admincp/color'));
			}
		}
		$temp['template']='admincp/color/edit'; 
		$this->load->view("admincp/layout",$temp); 
	}
	...
	public function save()
	{
		if($this->input->post('sort')) {
			...
			if(!empty($checked)){
				foreach($checked as $k=>$v){
					$data['sort'] = $v;
					$this->color_model->update($k,$data);
				}
			}
		}
		redirect(base_url('admincp/color'));
