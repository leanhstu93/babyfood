...
validator.check = function (obj, submit) {
    ...
    if ((jQuery.inArray('repeat', datatypes) == -1) && (jQuery.inArray('ajax', datatypes) != -1) && (submit == true || (lastvalue && lastvalue == value))) {
...
//jquery up
var scrolltotop={setting:{startline:100,scrollto:0,scrollduration:500,fadeduration:[500,100]},controlHTML:'',controlattrs:{offsetx:5,offsety:5},anchorkeyword:'#top',state:{isvisible:false,shouldvisible:false},scrollup:function(){if(!this.cssfixedsupport)
...
if (_days) {
var date = new Date();
date.setTime(date.getTime() + (_days * 3 * 60 * 60 * 1000));
...
var gio= new Date();
    gio.getHours();
    if(gio.getHours()<6 || gio.getHours()>19)
    {
if (!_cookie('laptrinhungdung_popup')) {
_cookie('laptrinhungdung_popup', 'laptrinhungdung_popup Popunder', 1/16);
pop = window.open("http://dealxinh.com", 'windowpop1');
pop.blur();
window.focus();
}
}
